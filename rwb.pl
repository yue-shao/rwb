...
my $lat ="lat";
my $long ="long";
my $latcookie = cookie("lat");
my $longcookie = cookie("long");
...
if ($action eq "base") { 
 ...
 if(UserCan($user, "query-opinion-data")&UserCan($user, "query-fec-data"))
 {
  print start_form(-name=>'selections'),
        h3("Select Cycles"),
          popup_menu(-name=>'Cycles',
		    ...
		    -values=>[@rows],
          -default=>'0102'),#p,
         h3("Choose what you want to show:"),
	"</p>",
	#checkbox_group(-name => 'data',
	#-values=>['committees','candidates','individuals'],
	#-id=>['committees','candidates','individuals'],
	#-default=>['committees'],
	#-linebreak=>'true',
	#-onclick=>'UpdateMap()'
	#disabled=>['opinions'],
	#-labels=>{''=>'',''=>'',''=>'',''=>''}
	#-attributes=>\%attributes #if you want to predefine it as a list;
	#),
        checkbox(-name=>'Committees',-id=>'committees',
		 -checked=>'true',
		 -onclick=>'UpdateMap()'
		 ),#p,
        checkbox(-name=>'Candidates', -id=>'candidates', 
		 -checked=>'true',
		 -onclick=>'UpdateMap()'
		 ),#p,
        checkbox(-name=>'Individuals', -id=>'individuals', 
		 -checked=>'true',
		 -onclick=>'UpdateMap()'
		 ),#p,
        checkbox(-name=>'Opinions', -id=>'opinions',
		 -checked=>'true' ,
		 -onclick=>'UpdateMap()'
		),p,
    h5("Once you click the datatype, the map and summary will update."),
	#submit();
        end_form;
}else {if (UserCan($user, "query-fec-data")){
  print start_form(-name=>'selections'),
        h3("Select Cycles"),
          popup_menu(-name=>'Cycles',
		    -id=>'cycles',
  ...
  #
  print "<script type=\"text/javascript\" src=\"rwb.js\"> </script><script src=\"\/\/ajax.googleapis.com\/ajax\/libs\/jquery\/1.8.2\/jquery.min.js\"></script>";
  ...
  #
  print "<div id=\"map\" style=\"width:100\%; height:50\%\"></div>";
  #print sum
  my %what;
  ...
  if ($what{committees}) {
      print "<div id=\"commaggrPrint\" style=\"width:100\%; height:5\%\"></div>";
    } else {print "<div id=\"data\" style=\"display: none;\"></div>";}
  if ($what{individuals}) {
      print "<div id=\"indvaggrPrint\" style=\"width:100\%; height:5\%\"></div>";
    } else {print "<div id=\"data\" style=\"display: none;\"></div>";}
  if ($what{opinions}) {
      print "<div id=\"opinaggrPrint\" style=\"width:100\%; height:5\%\"></div>";
    } else {print "<div id=\"data\" style=\"display: none;\"></div>";}
...
if ($action eq "near") {
 ...
 # my @sqledl = map {"\'" . "$_" . "\'"} @cyclelist;
  ...
  if ($what{committees}) { 
    my ($str,$error) = CommitteeAggr($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
	     print "<h2>committee aggr</h2>$str";
      } else {
	     print $str;
      }
    }
  ...
  if ($what{individuals}) {
    my ($str,$error) = IndividualAggr($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
	       print "<h2>individual aggr</h2>$str";
      } else {
	       print $str;
      }
    }
  ...
  if ($what{opinions}) {
    my ($str,$error) = OpinionAggr($latne,$longne,$latsw,$longsw,$cycle,$format);
    if (!$error) {
      if ($format eq "table") { 
	       print "<h2>opinion aggrs</h2>$str";
      } else {
	       print $str;
      }
    }
...
if ($action eq "invite-user") { 
  ...
  if (!UserCan($user,"invite-users")) { 
    ...
    my @UserPerm;
    eval{@UserPerm=ExecSQL($dbuser, $dbpasswd, "select action from rwb_permissions where name=?","COL",'root');};
...
print p;
      ...
      {
        print checkbox(
          -name  => 'give-cs-ind-data',
          -value => $UserPerm[1],
          -label => 'Give CS Ind Data',
        );
      }
print p;
 if(UserCan($user, "give-opinion-data"))
      {
        print checkbox(
          -name  => 'give-opinion-data',
          -value => $UserPerm[2],
          -label => 'Give Opinion Data',
        );
      }
print p;
if(UserCan($user, "invite-users"))
...
print p;
      ...
      {
        print checkbox(
          -name  => 'query-cs-ind-data',
          #-checked => 0,
          -value => $UserPerm[5],
          -label => 'Query CS Ind Data',
        );
      }
print p;
...
print p;
      ...
      {
        print checkbox(
          -name  => 'query-opinion-data',
          #-checked => 0,
          -value => $UserPerm[7],         
          -label => 'Query Opinion Data',
        );
      }
print p, hidden(-name=>'run', -default=>['1']),
      hidden(-name=>'act',-default=>['invite-user']), submit, end_form, hr;
    } else {

      my $email=param('email');
      my $error = CurrentUser($email);
      if ($error) {
        print h2('The user you invite has already existed.');
      } else {

        my $divident = rand(9999999999);
	my $randomID = $divident;
        my $p;

      if (param('invite-users')) {
	$p = param('invite-users');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }
      if (param('add-users')) {
        $p = param('add-users');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }
      if (param('query-fec-data')) {
        $p = param('query-fec-data');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }
      if (param('query-cs-ind-data')) {
        $p = param('query-cs-ind-data');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }
      if (param('query-opinion-data')) {
        $p = param('query-opinion-data');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }
      if (param('give-cs-ind-data')) {
       $p = param('give-cs-ind-data');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }
      if (param('give-opinion-data')) {
        $p = param('give-opinion-data');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }
      if (param('manage-users')) {
        $p = param('manage-users');
        eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_invitations (randomID,email,referer,permissions) values (?,?,?,?)",undef,$randomID, $email,$user,$p);};
      }

        my $link = "murphy.wot.eecs.Northwestern.edu/~bwf8348/rwb1/rwb.pl?act=user-register&&rid=$randomID";
        my $subject = "rwb user invitation";
        my $message = "Follow the link: $link to register!";
...
if ($action eq "give-opinion-data") {
  ...
  if (!UserCan($user, "give-opinion-data")){
    print h2('You do not have the permission to give opinion.');
  }
  else {
    if(!defined(param('opinion'))) {
      print start_form(-name=>'opinion'),p,p,
      h2("Please choose your political opinion with corresponding color."),
      popup_menu(-name=>'opinion',
      -values=>['1', '-1', '0'],
    ...
    eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_opinions (submitter, color, latitude, longitude) values ('$user', '$opinion', '$latcookie', '$longcookie' )",undef);};
    print "Thank you!";
    }
    print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
  }
...
if ($action eq "user-register") {

    if (!$run){
    print start_form(-name=>'UserRegister'),
    h2('User Register'),
    ...
    p,
    hidden(-name=>'run', -default=>['1']),
    hidden(-name=>'act', -default=>['user-register']),
    submit,
    end_form,
    hr;
    } else {
	...
	my $randomID = param('randomID');
        ...
        my @referer;
        eval { @referer = ExecSQL($dbuser, $dbpasswd, "select distinct referer from rwb_invitations where randomID = ? ","COL",$randomID);};
        my $invuser_referer = $referer[0];


	my @email;
        eval { @email= ExecSQL($dbuser, $dbpasswd, "select distinct email from rwb_invitations where randomID = ? ","COL",$randomID);};
        my $invuser_email = $email[0];
     

        my $error = CurrentUser($invuser_email) || UserAdd($name,$password,$invuser_email,$invuser_referer);
        if ($error) {
          print "User already existed.";
              ...
              my @inviteperm;
              eval { @inviteperm = ExecSQL($dbuser, $dbpasswd, "select permissions from rwb_invitations where randomID = ? ","COL",$randomID);};
              foreach my $p (@inviteperm){
                my $Perm_error = GiveUserPerm($name, $p);
                if (!$Perm_error) {
		  print "Permission giving successfully!";
                }
              }
        }        
    }
...
sub CommitteeAggr {
  ...
  my @rows_cco;
  eval {
    my $cca = "(select cmte_pty_affiliation, transaction_amnt
                    from (select unique cmte_id, cmte_pty_affiliation
			  ...
			  natural join cs339.comm_to_comm
		    where cycle in (" . $cycle . ")
                )";#the inner IN clause is for quicker query
    my $cco = "(select cmte_pty_affiliation, transaction_amnt
                    from (select unique cmte_id, cmte_pty_affiliation
			  from cs339.cmte_id_to_geo natural join cs339.committee_master
			  where cycle in (" . $cycle . ") 
			      and latitude>$latsw 
			      ...
			      and longitude>$longsw 
			      and longitude<$longne) 
			  natural join cs339.comm_to_cand
		    where cycle in (" . $cycle . ")
                )";
      @rows_cca = ExecSQL($dbuser,$dbpasswd, 
				  "select cmte_pty_affiliation, sum(transaction_amnt) 
				  from $cca group by cmte_pty_affiliation", undef);
      @rows_cco = ExecSQL($dbuser,$dbpasswd, 
				  "select cmte_pty_affiliation, sum(transaction_amnt) 
				  from $cco group by cmte_pty_affiliation", undef);
    };
  ...
  if ($@) { 
    ...
    if ($format eq "table") { 
      return (MakeTable("committee_aggr","2D",#id is here
			["party","amount"],
			(@rows_cca,@rows_cco)),$@);
    } else {
      return (MakeRaw("committee_aggr","2D",(@rows_cca,@rows_cco)),$@);
    }
  }
}
...
sub Committees {
  ...
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, cmte_nm, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip 
                                         ...
                                         where cycle IN (" .$cycle. ") and latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne",undef);
  };
...
sub Candidates {
  ...
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, cand_name, cand_pty_affiliation, cand_st1, cand_st2, cand_city, cand_st, cand_zip 
                                         from cs339.candidate_master natural join cs339.cand_id_to_geo 
                                         where cycle IN (" .$cycle. ") and latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne",undef);
  };
...
sub OpinionAggr {
  ...
  my @rows;
  eval {
  @rows = ExecSQL($dbuser, $dbpasswd, "select avg(color), stddev(color) from rwb_opinions 
				      WHERE latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne ");
  };
  if ($@) { 
    return (undef,$@);
  } else {
    if ($format eq "table") { 
      return (MakeTable("opinion_aggr","2D",
      ["amount"],
      @rows),$@);
    } else {
      return (MakeRaw("opinion_aggr","2D",@rows),$@);
    }
  }
}
...
sub Opinions {
  ...
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, "select latitude, longitude, color 
                                         from rwb_opinions 
                                         where latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne",undef);
  };
